...
from neuralnet.augmentations import add_disturbance

def load_and_augment_audio(file_path, sr=16000, apply_augmentation=True):
    """
    Load an audio file and optionally apply data augmentation.
    ...
    Returns:
        np.ndarray, int: Augmented audio signal and sample rate.
    """
    audio, sr = librosa.load(file_path, sr=sr)
    if apply_augmentation:
        audio = add_disturbance(audio, sr)
    return audio, sr
